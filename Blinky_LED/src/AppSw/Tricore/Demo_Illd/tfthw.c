...
void ILI9341_DrawColorBurst(uint16 color, uint32 size)
{
	uint32 BufferSize = 0;
	...
	uint32 RemainderFromBlock = SendingSize % BufferSize;
	...
	if(SendingInBlock != 0)
	{
		for(uint32 j = 0; j < (SendingInBlock); j++)
		{
                ...
                IfxQspi_SpiMaster_exchange(&g_Qspi_Tft.drivers.spiMasterChannel, BurstBuffer, 0, BufferSize);
